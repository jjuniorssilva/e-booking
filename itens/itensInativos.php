...
<form method="POST" action="validacoes/validar_busca_Item_inativo.php">
	Buscar pela descrição:
	<input type = 'text' name = 'buscarItem' title = 'Buscar Item' required>
	<input type = 'Submit' value = 'Enviar'>
	<a href="formularios/NovoItem.php"> cadastrar novo item</a>
</form>
<h4>ITENS INATIVOS</h4>
<?php
...
if (!$result) {
	...
	while (($linha = mysqli_fetch_array($result))==true) {
		...
		do {
			...
			$itens[$i][9]="<form method='POST' action='formularios/form_alterar_item.php'><button type='submit' name='alterarInativo' value='$i'>ALTERAR</button></form>";
			$itens[$i][10]="<form method='POST' action='excluir_item.php'><button type='submit' name='excluirInativo' value='$i'>EXCLUIR</button></form>";
			$i++;
		} while ($i<0);
	...
	<table>
		...
		if ($i!=1) {
			...
			echo "NENHUM ITEM INATIVO NO MOMENTO!";
		}
	}
	mysqli_close($conexao); 
	?>
</table>
<a href="itens.php">Ver Itens Ativos</a>