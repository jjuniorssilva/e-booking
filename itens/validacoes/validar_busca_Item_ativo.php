...
if (!$result) {
	...
	while (($linha = mysqli_fetch_array($result))==true) {
		...
		do {
			...
			$itens[$i][7]=$obs_item;
			$itens[$i][8]="<form method='POST' action='../inativar_item.php'><button type='submit' name='inativarBuscaAtivo' value='$i'>INATIVAR</button></form>";
			$itens[$i][9]="<form method='POST' action='../formularios/form_alterar_item.php'><button type='submit' name='alterarBuscaAtivo' value='$i'>ALTERAR</button></form>";
			$itens[$i][10]="<form method='POST' action='../excluir_item.php'><button type='submit' name='excluirBuscaAtivo' value='$i'>EXCLUIR</button></form>";
			$itens[$i][11]="<form method='POST' action='../formularios/reservar_item.php'><button type='submit' name='reservarItem' value='$i'>RESERVAR</button></form>";
			$i++;
		} while ($i<0);
	}
}
$_SESSION['$buscaitensAtivos']=$itens;
?>
<table>
	<tr>
...
<table>
	...
	if ($i!=1) {
		...
		echo "<script type='text/javascript'>alert ('NENHUM ITEM ENCONTRADO!'); window.location.href='../itens.php';</script>";
	}
	mysqli_close($conexao);
	?>
</table>